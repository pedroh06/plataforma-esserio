...
export default function Medicina() {
  ...
  React.useEffect(() => {
    async function sendForm() {
      if(formData.text === "" || formData.text === "<h3>Nutrição</h3>") {
        return;
      }
      await axios.post(`/api/fichas/nutricaoBlank/${fichaId}`, {
        ...formData,
      });
    ...
    const getResident = async () => {
      ...
      try {
        const [residentsRaw, response, blank_sheetRaw] = await Promise.all([
          ...
          fetch(`/api/fichas/nutricaoBlank/${fichaId}`),
        ]);
  ...
  return (
    <RecordLayout>
      <div className="my-4 flex w-full flex-col  min-h-screen  items-center justify-center gap-4 rounded-md bg-white px-4 py-16 shadow-2xl sm:w-[600px]">
        <h1 className="bold text-xl">Nutrição - Ficha em Branco</h1>
