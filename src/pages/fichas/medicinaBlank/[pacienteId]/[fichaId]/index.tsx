...
export default function Medicina() {
  ...
  const debouncedSearchTerm = useDebouncedValue(formData.text, 1500);


  React.useEffect(() => {
    async function sendForm() {
      if(formData.text === "" || formData.text === '<h3>Enfermagem</h3><ul><li><p>Queixa: </p></li><li><p>Ao exame físico: </p></li><li><p>PA: </p></li><li><p>Temperatura:</p></li><li><p>SPO2:</p></li><li><p>Pulso:  </p></li></ul><h3>Medicina</h3><ul><li><p>HDA</p></li><li><p>AMP: </p></li><li><p>Conduta:</p></li></ul>') {
        return;
      }
      await axios.post(`/api/fichas/medicinaBlank/${fichaId}`, {
        ...formData,
      });
    ...
    const getResident = async () => {
      ...
      try {
        const [residentsRaw, response, blank_sheetRaw] = await Promise.all([
          ...
          fetch(`/api/fichas/medicinaBlank/${fichaId}`),
        ]);
  ...
  return (
    <RecordLayout>
      <div className="my-4 flex w-full flex-col  min-h-screen  items-center justify-center gap-4 rounded-md bg-white px-4 py-16 shadow-2xl sm:w-[600px]">
        <h1 className="bold text-xl">Medicina - Ficha em Branco</h1>
